        ...
        <div>
          ...
          <div className="space-y-1">
            {skills.filter(s => s.enabled).map((skill) => (
              <div key={skill.path} className="text-sm py-1.5 px-2 rounded bg-secondary">
                {skill.name}
              </div>
            ))}
        ...
        <div>
          ...
          <div className="space-y-1">
            {mcpServers.map((server) => (
              <div key={server.name} className="text-sm py-1.5 px-2 rounded bg-secondary">
                {server.name}
              </div>
            ))}
            {mcpServers.length === 0 && (
              <p className="text-xs text-muted-foreground">暂无连接的服务</p>
            )}
          </div>
        </div>
      </div>
