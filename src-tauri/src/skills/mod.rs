...
pub fn list_skills() -> Result<Vec<SkillInfo>> {
    ...
    for setting in &config.skills {
        ...
        if path.exists() {
            let definition = load_skill_definition(&path).ok();
            let name = definition.as_ref()
                .map(|d| d.name.clone())
                .unwrap_or_else(|| path.file_name()
                    .map(|n| n.to_string_lossy().to_string())
                    .unwrap_or_default());
            
            let description = definition.as_ref()
...
pub fn load_skill_definition(skill_path: &PathBuf) -> Result<SkillDefinition> {
    ...
    if json_path.exists() {
        ...
        Err(anyhow::anyhow!("No skill.json found"))
    }
}
...
pub fn import_local_skill(source_path: &str) -> Result<SkillInfo> {
    ...
    let definition = load_skill_definition(&target).ok();
    let name = definition.as_ref()
        .map(|d| d.name.clone())
        .unwrap_or(skill_name.clone());
...
pub fn import_from_github_repo(repo_url: &str) -> Result<SkillInfo> {
    ...
    // Note: In production, use git2 crate for cloning
